...
#                        WIoU=False, ShapeIoU=False,Inner = False,ratio = 0.75, eps = 1e-7, scale = 0.0)
#
...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask,hw):
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask],          # 当使用SDIoU损失函数时，令d=0.3,0.5,0.7都可以
                       xywh=False, GIoU=False, DIoU=False, CIoU=True, EIoU=False, SIoU=False,SDIoU=False,d=0.3,
                       WIoU=False,ShapeIoU=False, hw=hw[fg_mask], mpdiou=False, Inner=False, ratio=0.75,
                       eps=1e-7, scale=0.0)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max - 1, use_dfl=self.use_dfl,nwd_loss=self.nwdloss,iou_ratio=self.iou_ratio).to(device)
